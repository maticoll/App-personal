...
- **IA:** Codex API (Anthropic) — orquestación, NLP, macros, ideas
...
Cada sesión genera un `skill.md` propio y agrega su bloque a este `AGENTS.md`.
...
**lib/nutrition.ts:** `getTodayNutritionSummary`, `getMealHistory`, `getUserDiet`, `updateUserDiet`, `logMealNLP` (Codex Haiku calcula macros + dietAlignmentScore), `logWater`, `deleteMeal`, `getWeeklyNutritionStats`, `getNutritionSummaryText`, `getWaterReminderText`. Normalización NFD para regex sin acentos en agentes.

**lib/ideas.ts:** `getAllIdeas` (con filtros tag/search), `getRecentIdeas`, `getIdea`, `captureIdeaNLP` (Codex Haiku estructura: title + content expandido + tags), `updateIdea`, `deleteIdea`, `getIdeasStats`, `getIdeasActivityForDate` (informativo, no entra al score).
...
**lib/orchestrator.ts:** Orquestrador central. Recibe `(userId, messageText)` y llama al agente correspondiente segun la intencion detectada por Codex Haiku. Devuelve string de respuesta.
...
6. Cierre motivacional: Codex Haiku (max 60 tokens, 1 linea, sin markdown)
...
**lib/orchestrator.ts:** `orchestrate(userId, text)` - Codex Haiku clasifica en 7 modulos y deriva al agente correcto.
...
**agents/calendar/index.ts:** Reemplaza el stub vacio. Detecta 4 intenciones: `query_today`, `query_week`, `create_event`, `status`. `create_event`: usa Codex Haiku para parsear título + fecha + hora desde texto libre. Exporta `calendarAgent.getTodayEventsText()`, `calendarAgent.findFreeSlots()`, `calendarAgent.createEvent()` para uso de otros agentes.
...
**agents/finances/index.ts:** Reemplaza el stub. Detecta 5 intenciones: `query_spending`, `query_balance`, `query_report`, `create_expense`, `create_income`. Parseo de transacciones en lenguaje natural via Codex Haiku. `checkSpendingAlerts`: alerta si los gastos subieron >20% vs mes anterior.
...
- `summarizeTurns()` — llama Codex Haiku (≤120 palabras, tercera persona, espanol rioplatense, incorpora resumen anterior)
...
3. Codex Haiku clasifica modulo (rapido, barato)
4. Agente especialista ejecuta accion y retorna datos crudos
5. Codex Sonnet genera respuesta final con voz natural + contexto
...
**`generateFinalResponse()`:** Codex Sonnet (Codex-sonnet-4-6), max_tokens: 350. Voz rioplatense, sin asteriscos, max 3-4 oraciones para respuestas simples.
...
**`buildDataSummary(data)`:** Formatea datos para Codex: periodos, registros de sueno, workouts, comidas, scores diarios, proyectos.

**`callSynthesisAI(systemPrompt, dataSummary)`:** Codex Sonnet, max 3 patrones/conexiones, recomendacion concreta, sin markdown.
...
**Funciones internas:**
- `extractTransaction(text, categories)`: Codex Haiku con lista completa de categorias en el prompt. Extrae la transaccion en JSON estructurado.
...
Antes de clasificar el modulo con Codex Haiku, el orquestrador chequea si hay una transaccion pendiente:

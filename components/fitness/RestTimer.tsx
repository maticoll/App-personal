...
export default function RestTimer({ seconds, onDone, onSkip, onAdjust }: Props) {
  // `seconds` se lee solo al montar; el parent remonta (key=nonce) en cada
  // nuevo descanso, así que no hace falta sincronizar con un effect (eso
  // reiniciaba la cuenta al ajustar). Ajustar suma/resta al tiempo RESTANTE.
  ...
  const adjust = (delta: number) => {
    ...
    onAdjust(delta); // persiste el default del ejercicio para la próxima serie
  };
  ...
  return (
    <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-[#06B6D4]/15 text-accent-cyan rounded-full px-3 py-2 backdrop-blur">
      <button onClick={() => adjust(-15)} className="px-2 font-bold">-15</button>
